...
def attacked_queens_pairs(seqs):
    ...
    """
    a = np.array([0] * N**2)  # Create a one-dimensional array with n**2 zeros
    a = a.reshape(N, N)  # Change to n * n two-dimensional array. For the convenience of later use, only the n-1 * n-1 parts of the last eight rows and columns are used as a blank chessboard
    ...
    for i in range(1, N):
        ...
        for k in list(range(1, i)) + list(range(i + 1, N)):  # Check whether there are other queens on each queen's line
        ...
        t1 = t2 = seqs[i - 1]
        for j in range(i + 1, N):  # Look at the two diagonals in the right half
            if t1 != 1:
                t1 -= 1
                if a[t1][j] == 1:
...
def display_board(seqs):
    ...
    """
    board = np.array([0] * N**2)  # Create a one-dimensional array with n**2 zeros
    board = board.reshape(N, N)  # Change to a n * n two-dimensional array. For the convenience of later use, only the n-1 * n-1 parts of the last eight rows and columns are used as a blank chessboard
...
start = time.time()
frontier_priority_queue = [{'unplaced_queens':N-1, 'pairs':28, 'seqs':[0] * N-1}] # The initial state is 8 zeros, which means that there are no queens on the chessboard; g(n) = the number of queens that have not been placed well, h(n) = the logarithm of queens attacking each other, and h(n)=28, g(n)=8
solution = []
...
while frontier_priority_queue: # If the frontier is not empty, the loop will continue. If the solution is found successfully, the loop will output the solution. If the frontier is empty, the loop will fail
    ...
    if first['pairs'] == 0 and first['unplaced_queens'] == 0: # Do the goal test before extending the node: if the sequence h(n)=g(n)=0, then the sequence is the solution sequence
        ...
        flag = 1  # success
        break
    nums = list(range(1, N))  # List with elements 1-8
    seqs = first['seqs']
    ...
    for j in range(N-1): # In the first position of 0 in the sequence, that is, the leftmost column without queen, select a row to place queen
        pos = seqs.index(0)
        temp_seqs = list(seqs)
